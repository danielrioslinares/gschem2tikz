...
class Node:
	...
	def draw(self):
		...
		text = '{\\scalebox{' + str(int(self.size)/27) + '}{' + str(self.text) + '}}'
...
class Pin:
	...
	def __init__(self, XY1, XY2, color = None, headcolor = None, thickness = None, headthickness = 2, showPinHead = True):
		self.XY1 = XY1
		self.XY2 = XY2
		self.color = color
...
class TikZComponent:
	...
	def __init__(self, XY, tikz_scope, rotate = 0, mirror = False, colors = {}, thickness = {}):
		self.XY = XY
		self.rotate = rotate
		self.mirror = mirror
		...
		if self.mirror:
			...
			while notyet:
				...
				if (int(rotate) == 0 or int(rotate) == 180):
					tikz_scope = tikz_scope.replace('&!left!&','right')
					tikz_scope = tikz_scope.replace('&!right!&','left')
				elif (int(rotate) == 90 or int(rotate) == 270):
					tikz_scope = tikz_scope.replace('&!above!&','below')
					tikz_scope = tikz_scope.replace('&!below!&','above')
				if tikz_scope == last_tikz_scope: notyet = False
		tikz_scope = tikz_scope.replace('&!rotate!&',str(self.rotate))
		for color in self.colors:
			tikz_scope = tikz_scope.replace('&!' + color[3:] + '!&', colors[color])
		for thick in self.thickness:
			tikz_scope = tikz_scope.replace('&!' + thick[5:] + '!&', thickness[thick])
		draw += tikz_scope.replace('\\begin{tikzpicture}','').replace('\\end{tikzpicture}','')
		draw += '\\end{scope}\n'
		return draw
