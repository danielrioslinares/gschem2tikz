...
class Schematic:
		...
		for line in data:
			try:
				...
				if line[0] == 'T':
					...
					try:
						if child_layer:
							for lst in gc.get_cfg('maintab/attributes','attributestable','list'):
								if attr == lst[0]:
									shown = True if lst[2] == 'True' or lst[2] == 'True' else False
									line[10] = line[10].split('=')[1]
						# Hide certain attributes outside components
						else:
							for lst in gc.get_cfg('maintab/attributes','attributestable','list'):
								if attr == lst[0]:
									shown = True if lst[1] == 'True' or lst[1] == 'True' else False
									line[10] = line[10].split('=')[1]
					except:
						shown = True
				...
				if line[0] == 'P':
					...
					dic = {
						...
						'showpinhead' : gc.get_cfg('maintab/general/otherbox/layout','boolshowpinhead','bool') ,
					}
					pinss.append(dic)
				# Component
				if line[0] == 'C':
	...
	def dic2shapes(self, texts, lines, buses, netss, boxes, picts, arcss, comps, tikz_comps, circs, pinss):
		...
		if len(buses) > 1:
			plines_buses = PolyLines( buses, color = buses[0]['color'], thickness = buses[0]['thickness'] )
			for bus in plines_buses.line_list:
				tikz_shapes.append(bus)
		else:
			for bus in buses:
				tikz_shapes.append( Line(bus['XY1'], bus['XY2'], bus['color'], bus['thickness']) )

		if len(netss) > 1:
			plines_netss = PolyLines( netss, color = netss[0]['color'], thickness = netss[0]['thickness'] )
			for net in plines_netss.line_list:
				tikz_shapes.append(net)
		else:
			for net in netss:
				tikz_shapes.append( Line(net['XY1'], net['XY2'], net['color'], net['thickness']) )
		...
		for pin in pinss:
			tikz_shapes.append( Pin(pin['XY1'], pin['XY2'], pin['color'], pin['headcolor'], pin['thickness'], pin['headthickness'], pin['showpinhead']) )
